...
bool Generic_BME280::begin(uint8_t addr)
{
    ...
    // check if sensor, i.e. the chip ID is correct
		uint8_t foo = read8(BME280_REGISTER_CHIPID);
		LOG (LL_INFO, ("GAA Register id is 0x%x", foo));
    ...
    while (isReadingCalibration()) {
			LOG (LL_INFO, ("chip reading calibrationm data"));
      mgos_msleep(100);
		}
    readCoefficients(); // read trimming parameters, see DS 4.2.2
		LOG (LL_INFO, ("GAA - dig_T1: 0x%x", _bme280_calib.dig_T1));
...
void Generic_BME280::setSampling(sensor_mode       mode,
    ...
    // CONTROLHUMID register, otherwise the values won't be applied (see DS 5.4.3)
		LOG(LL_INFO, ("Before set, humReg: 0x%x, configReg: 0x%x, measReg: 0x%x",
									read8(BME280_REGISTER_CONTROLHUMID),
									read8(BME280_REGISTER_CONFIG),
									read8(BME280_REGISTER_CONTROL)));
    write8(BME280_REGISTER_CONTROLHUMID, _humReg.get());
    write8(BME280_REGISTER_CONFIG, _configReg.get());
    write8(BME280_REGISTER_CONTROL, _measReg.get());
		LOG(LL_INFO, ("AFTER set, humReg: 0x%x, configReg: 0x%x, measReg: 0x%x",
									read8(BME280_REGISTER_CONTROLHUMID),
									read8(BME280_REGISTER_CONFIG),
									read8(BME280_REGISTER_CONTROL)));
}
...
void Generic_BME280::write8(byte reg, byte value) {
	if (_protocol == I2C) {
		LOG(LL_INFO, ("writing 0x%x to register 0x%x", value, reg));
...
uint16_t Generic_BME280::read16(byte reg)
{
  uint16_t retval = 0xFFFF;
	...
	LOG(LL_INFO, ("GAA before protocol read, protocol set to %d", _protocol));
  if (_protocol == I2C) {
		// read first register
		readval = mgos_i2c_read_reg_b(_i2c, _i2caddr, reg);
		retval = readval << 8 | mgos_i2c_read_reg_b(_i2c, _i2caddr, reg+1);
		LOG(LL_INFO, ("GAA read16 value: 0x%x", retval));
  } else {
...
uint32_t Generic_BME280::read24(byte reg)
{
  uint32_t  retval = 0xFFFFFFFF;
	...
	LOG(LL_INFO, ("GAA before protocol read, protocol set to %d", _protocol));
	if (_protocol == I2C) {
		LOG(LL_INFO, ("GAA just in protocol read"));
		readval = mgos_i2c_read_reg_b(_i2c, _i2caddr, reg);
		retval = readval << 8 | mgos_i2c_read_reg_b(_i2c, _i2caddr, reg+1);
		retval = retval << 8 | mgos_i2c_read_reg_b(_i2c, _i2caddr, reg+2);
		LOG(LL_INFO, ("GAA read24 value: 0x%x", retval));
  } else {
		LOG(LL_INFO, ("GAA ended up in SPI"));
		// SPI transaction placeholder
  }
  return retval;
}
...
void Generic_BME280::takeForcedMeasurement()
{
    ...
    if (_measReg.mode == MODE_FORCED) {
				LOG(LL_INFO, ("GAA In forced mode..."));
